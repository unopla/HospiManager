...
def criar_tela_medico(nome_usuario):
    ...
    # PAINEL DA FILA
    # -------------------
    fila_frame = ctk.CTkFrame(main_frame, fg_color=COLORS["card"], corner_radius=14, width=260)
    fila_frame.grid(row=0, column=0, sticky="ns", padx=(0, 20), pady=10)

    ctk.CTkLabel(fila_frame, text="Fila de Atendimento", font=FONT_TITLE, text_color=COLORS["primary"]).pack(pady=15)

    lista_fila = ctk.CTkTextbox(fila_frame, fg_color=COLORS["primary_light"], corner_radius=12, width=250)
    ...
    ctk.CTkLabel(paciente_frame, text="Paciente em Atendimento", font=FONT_TITLE, text_color=COLORS["primary"]).pack(pady=12)

    info_label = ctk.CTkLabel(paciente_frame, text="", font=FONT_LABEL, justify="left", anchor="w")
    ...
    # FUNÇÕES DE BANCO
    # ===================
    def localizar_id_medico(conn, login_medico):
    ...
    def finalizar_atendimento():
        ...
        try:
            ...
            """, (
                p['id_paciente'], id_med, p['id_triagem'], diagnostico,
                exame + "\n\nAnamnese:\n" + anamnese,
                'Consulta', datetime.now(), 'Finalizado'
            ))
            ...
            for proc in procedimentos_txt:
                if proc.strip():
                    cursor.execute("""
                        ...
                        VALUES (%s, %s, %s, NOW(), 'Médico')
                    """, (id_atend, proc, nome_usuario))
            ...
            cursor.execute("UPDATE triagem SET status='Finalizado' WHERE id_triagem=%s", (p['id_triagem'],))
            conn.commit()
    ...
    ctk.CTkLabel(footer, text="© Hospi Manager — Sistema Clínico de Gestão", text_color="white", font=("Arial", 11)).pack(pady=4)
